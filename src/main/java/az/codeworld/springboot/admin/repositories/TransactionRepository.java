...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    List<Transaction> findAllTransactionsByBelongsTo(roles belongsTo, Pageable pageable);
    ...
    @Query("""
        SELECT
            YEAR(t.transactionTime) AS year,
            MONTH(t.transactionTime) AS month,
            COUNT(t) AS txCount,
            COALESCE(SUM(t.transactionTotal), 0) AS amount
        FROM Transaction t
        WHERE t.belongsTo = :role
        AND t.status = :status
        AND t.transactionTime >= :startTime
        AND t.transactionTime < :endTime
        GROUP BY YEAR(t.transactionTime), MONTH(t.transactionTime)
        ORDER BY YEAR(t.transactionTime), MONTH(t.transactionTime)
    """)
    List<MonthlyTransactionProjection> getMonthlyAmount(
